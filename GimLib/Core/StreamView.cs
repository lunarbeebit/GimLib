...
public class StreamView : Stream
{
    ...
    public override int ReadByte()
    {
        ...
        if (positionInBaseStream >= endInBaseStream) return -1;
    ...
    ///     <paramref name="offset" /> + <paramref name="count" /> - 1) replaced by the characters read from the current
    ///     stream.
    /// </param>
    /// <param name="offset">
    ///     The zero-based byte offset in <paramref name="buffer" /> at which to begin storing data from the
    ///     current stream.
    /// </param>
    /// <param name="count">The maximum number of bytes to read.</param>
    /// <returns>
    ///     The total number of bytes written into the buffer. This can be less than the number of bytes requested if that
    ///     number of bytes are not currently available, or zero if the end of the stream is reached before any bytes are read.
    /// </returns>
    public override int Read(byte[] buffer, int offset, int count)
    {
        ...
        if (positionInBaseStream >= endInBaseStream) return 0;
        if (positionInBaseStream + count > endInBaseStream) count = (int)(endInBaseStream - positionInBaseStream);
    ...
    /// </summary>
    /// <param name="offset">
    ///     The new position within the stream. This is relative to the <paramref name="origin" /> parameter,
    ...
    public override long Seek(long offset, SeekOrigin origin)
    {
        ...
        switch (origin)
        {
            ...
            case SeekOrigin.End:
            {
                if (endInBaseStream + offset < startInBaseStream) throw new ArgumentOutOfRangeException(nameof(offset));
    ...
    /// <summary>
    ///     Releases the unmanaged resources used by the <see cref="StreamView" /> class and optionally releases the managed
    ///     resources.
    /// </summary>
    ...
    {
        if (disposing && !disposed) disposed = true;
